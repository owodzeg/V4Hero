...
TEST_CASE("save file deserialization", "is ok")
{
...
TEST_CASE("save file serialization", "is ok")
{
    std::ifstream conf("test-resources/new_save.p4sv", std::ios::in);
    json expected_serialized_data;
    conf >> expected_serialized_data;

    SaveFile save_file = SaveFile{
        .save = SaveData{
            .army = Army{
                .hero = SquadSlot{
                    .rarepon = -1,
    ...
    json serialized_data = save_file;
    REQUIRE(serialized_data == expected_serialized_data);
}
