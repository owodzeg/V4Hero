...
void ObeliskMenu::addMission(json missiondata)
{
    ...
    {
        tmp.mis_ID = 0;
        SPDLOG_ERROR("Exception when adding mission: e.what():", e.what());
    }
    try
    {
...
void ObeliskMenu::Update()
{
    ...
    v_render_map.setViewport(sf::FloatRect(sf::Vector2f(134.0f*3 / 3840.0f, (mainbox.getPosition().y - 115*3) / 2160.0f), sf::Vector2f(1012.0f*3 / 3840.0f, 1.f)));
    ...
    if (displayMissions)
    {
        ...
        v_render_missions_map.setViewport(sf::FloatRect(sf::Vector2f(143.0f*3 / 3840.f, (missionbox.getPosition().y - 53*3) / 2160.0f), sf::Vector2f(298.0f*3 / 3840.0f, 120.0f*3 / 2160.0f)));
